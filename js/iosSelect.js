(function() {
	...
	function IosSelect(level, data, options) {
		...
		this.options.value = options.value || "value";
		this.options.id = options.id || "id";
	...
	IosSelect.prototype = {
		init: function() {
			...
			this.initLayer(function(){
			    setTimeout(self.iosSelectLayer.el.querySelector('.layer').classList.add("on"),2000);
			});
		...
		renderOneLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, oneLevelData) {
			...
			var hasAtId = oneLevelData.some(function(v, i, o) {
				return v[self.options.id] == oneLevelId;
			});
			if (!hasAtId) {
				oneLevelId = oneLevelData[0][self.options.id];
			}
			var oneHtml = '';
			
			var plast = 0;
			oneHtml += self.getWhiteItem();
			oneLevelData.forEach(function(v, i, o) {
				if (v[this.options.id] == oneLevelId) {
					oneHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';" ' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[this.options.value] + '</li>';
					plast = i + 1;
				} else {
					oneHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v[this.options.value] + '</li>';
				}
			}.bind(this));
			oneHtml += self.getWhiteItem();
			self.oneLevelUlContainDom.innerHTML = oneHtml;

			self.scrollOne.refresh();
			self.scrollOne.scrollToElement('li:nth-child(' + plast + ')', 0);
			if (self.level >= 2) {
				self.setTwoLevel(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId);
			}

			var pdom = self.changeClassName(this.oneLevelContainDom, plast);
			self.selectOneObj = iosSelectUtil.attrToData(pdom, self.getAtIndexByPlast(plast));
		},
